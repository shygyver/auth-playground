...
declare module "@saurbit/oauth2" {
  ...
  // include consent status in the user object for authorization code generation
...
// define the shape of parsed data from the authorization endpoint, including consent and session cookie
...
// changed: added "offline_access" to the client's scopes to allow requesting refresh tokens for offline access
// and "refresh_token" to the grants to allow using the refresh token grant type
const CLIENT: {
  id: string;
  secret: string;
  grants: string[];
...
// new: in-memory refresh token storage, mapping refresh tokens to client, user, scope, and expiration
...
const flow = HonoOIDCAuthorizationCodeFlowBuilder.create<Env, ParsedData>({
  // parse form data from the authorization endpoint to extract username, password, consent, and session cookie
  ...
  // changed: added "offline_access" to the list of available scopes
  .setScopes({
  ...
  .setDescription("Example OpenID Connect Authorization Code Flow with Refresh Token support")
  ...
  // changed: enhanced client retrieval to handle both authorization code and refresh token grant types
...
// handle GET requests to the authorization endpoint, checking for existing session
...
// handle form submission at the authorization endpoint,
